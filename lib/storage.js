...
export async function incrementApiCalls(walletAddress, apiType = 'trending') {
  if (!isAddress(walletAddress)) {
    throw new Error('Invalid wallet address');
  }
  await initializeSchema();
  ...
  try {
    await sql`
      INSERT INTO user_usage (wallet_address, usage_date, ${field})
      VALUES (${userKey}, CURRENT_DATE, 1)
      ON CONFLICT (wallet_address, usage_date)
      DO UPDATE SET ${field} = user_usage.${field} + 1`;
    return true;
  } catch (error) {
...
export async function getApiCallsUsed(walletAddress, apiType = 'trending') {
  ...
  try {
    const rows = await sql`
      SELECT ${field} FROM user_usage
      WHERE wallet_address = ${userKey}
      AND DATE_TRUNC('day', usage_date) = CURRENT_DATE`;
    return rows[0]?.[field] || 0;
  } catch (error) {
    throw new Error('Failed to get API calls used: ' + error.message);
  }
}
...
export async function rollbackApiCalls(walletAddress, apiType = 'trending') {
  ...
  try {
    await sql`
      UPDATE user_usage
      SET ${field} = GREATEST(user_usage.${field} - 1, 0)
      WHERE wallet_address = ${userKey}
      AND DATE_TRUNC('day', usage_date) = CURRENT_DATE`;
    return true;
  } catch (error) {
    throw new Error('Failed to rollback API calls: ' + error.message);
  }
}