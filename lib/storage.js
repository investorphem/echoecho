...
// Subscription durations from environment variables with fallback
  ...
  try {
    ...
    await sql`
      CREATE TABLE IF NOT EXISTS user_usage (
        ...
        ai_api_calls_used INTEGER DEFAULT 0,
        echoes_api_calls_used INTEGER DEFAULT 0,
    ...
    await sql`
      CREATE TABLE IF NOT EXISTS rate_limits (
        ...
        CONSTRAINT unique_user_action UNIQUE (user_key, action)
      )`;
...
export async function getUser(walletAddress) {
  ...
  try {
    const [user] = await sql`SELECT * FROM users WHERE wallet_address = ${userKey}`;
    return user || null;
  } catch (error) {
...
export async function createUser(walletAddress, userInfo = {}) {
  ...
  if (userInfo.tier && !validTiers.includes(userInfo.tier)) {
    throw new Error(`Invalid tier: ${userInfo.tier}`);
  }
...
export async function saveUserNotificationDetails(walletAddress, { token, url }) {
  ...
  if (!token || !url) {
    throw new Error('Missing notification token or URL');
  }
  await initializeSchema();
  const userKey = walletAddress.toLowerCase();
...
export async function createSubscription(walletAddress, tier, transactionHash) {
  ...
  const subscription = {
    ...
    amount_usdc: null, // To be set by saveSubscription
  };

  try {
    await sql`
      INSERT INTO subscriptions (
        id, user_id, wallet_address, tier, status, created_at, expires_at, next_billing_at, auto_renew, transaction_hash, amount_usdc
      )
      VALUES (
        ...
        ${subscription.auto_renew}, ${subscription.transaction_hash}, ${subscription.amount_usdc}
      )`;
...
export async function saveSubscription(walletAddress, tier, transactionHash, additionalData = {}) {
  ...
  if (additionalData.amount_usdc && (typeof additionalData.amount_usdc !== 'number' || additionalData.amount_usdc <= 0)) {
  ...
  const subscriptionId = `sub_${Date.now()}_${Math.random().toString(36).slice(2, 11)}`;
  const now = new Date();
  const expiresAt = additionalData.status === 'expired' ? now : new Date(now.getTime() + SUBSCRIPTION_DURATIONS[tier]);
  ...
  const subscription = {
    ...
    created_at: now.toISOString(),
    expires_at: expiresAt.toISOString(),
    next_billing_at: additionalData.status === 'expired' ? null : expiresAt.toISOString(),
    auto_renew: additionalData.auto_renew ?? true,
    last_reminder_3d_at: null,
    last_reminder_1d_at: null,
    transaction_hash: transactionHash,
    amount_usdc: additionalData.amount_usdc || null,
  };
  ...
  try {
    ...
    if (subscription) {
      await sql`
        ...
        WHERE wallet_address = ${subscription.wallet_address}`;
      return subscription;
    }
    return null;
  } catch (error) {
...
export async function getAllActiveSubscriptions() {
  await initializeSchema();
  try {
    const subscriptions = await sql`SELECT * FROM subscriptions WHERE status = 'active'`;
    return subscriptions;
  } catch (error) {
...
export async function saveEcho(echo) {
  ...
  if (!echo.cast_id || !echo.type || !echo.source || !echo.id) {
    throw new Error('Missing required echo fields');
  }
  await initializeSchema();
...
// API call tracking
export async function incrementApiCalls(walletAddress, apiType = 'trending', increment = 1) {
  if (!isAddress(walletAddress)) {
    throw new Error('Invalid wallet address');
  }
  const validApiTypes = ['trending', 'ai', 'echoes'];
  if (!validApiTypes.includes(apiType)) {
    throw new Error(`Invalid API type: ${apiType}`);
  }
  await initializeSchema();
  const userKey = walletAddress.toLowerCase();
  const field = `${apiType}_api_calls_used`;

  try {
    const [result] = await sql`
      INSERT INTO user_usage (wallet_address, usage_date, ${sql(field)})
      VALUES (${userKey}, CURRENT_DATE, ${increment})
      ON CONFLICT (wallet_address, usage_date)
      DO UPDATE SET ${sql(field)} = GREATEST(user_usage.${sql(field)} + ${increment}, 0)
      RETURNING ${sql(field)}`;
    return result[field];
  } catch (error) {
    throw new Error(`Failed to update API calls for ${apiType}: ${error.message}`);
  }
}

export async function getApiCallsUsed(walletAddress, apiType = 'trending') {
  if (!isAddress(walletAddress)) {
    throw new Error('Invalid wallet address');
  }
  const validApiTypes = ['trending', 'ai', 'echoes'];
  ...
  try {
    const [row] = await sql`
      SELECT ${sql(field)} FROM user_usage
      WHERE wallet_address = ${userKey}
      AND DATE_TRUNC('day', usage_date) = CURRENT_DATE`;
    return row?.[field] || 0;
  } catch (error) {
    throw new Error(`Failed to get API calls for ${apiType}: ${error.message}`);
  }
}

// Revenue tracking
export async function getTotalSubscriptionRevenue() {
  await initializeSchema();
  try {
    const [result] = await sql`
      SELECT COALESCE(SUM(amount_usdc), 0) as total_usdc
      FROM subscriptions
      WHERE amount_usdc IS NOT NULL`;
    return result.total_usdc || 0;
  } catch (error) {
    throw new Error(`Failed to calculate total subscription revenue: ${error.message}`);
  }
}

// New function to resolve the missing import
export async function rollbackApiCalls(walletAddress, apiType = 'trending') {
  if (!isAddress(walletAddress)) {
    throw new Error('Invalid wallet address');
  }
  const validApiTypes = ['trending', 'ai', 'echoes'];
  ...
  try {
    ...
    throw new Error(`Failed to rollback API calls for ${apiType}: ${error.message}`);
  }
}

// Rate limiting
export async function checkRateLimit(userKey, action, maxRequests = 5, windowSeconds = 3600) {
  if (!isAddress(userKey)) {
    throw new Error('Invalid wallet address');
  }
  await initializeSchema();
  const rateLimitKey = `${userKey}:${action}`;
  const id = `rl_${Date.now()}_${Math.random().toString(36).slice(2, 11)}`;

  try {
    // Reset count if window has expired
    const [result] = await sql`
      INSERT INTO rate_limits (id, user_key, action, request_count, last_reset)
      VALUES (${id}, ${userKey.toLowerCase()}, ${action}, 0, CURRENT_TIMESTAMP)
      ON CONFLICT ON CONSTRAINT unique_user_action
      DO UPDATE SET
        request_count = CASE
          WHEN rate_limits.last_reset < CURRENT_TIMESTAMP - INTERVAL '${windowSeconds} seconds'
          THEN 0
    ...
    const [updated] = await sql`
      ...
      WHERE user_key = ${userKey.toLowerCase()} AND action = ${action}
      RETURNING request_count`;

    if (updated.request_count > maxRequests) {
