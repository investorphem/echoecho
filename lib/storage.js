...
export async function saveNFT(nft) {
  if (!isAddress(nft.user_address)) throw new Error('Invalid wallet address');
  if (!nft.token_id || !nft.id) throw new Error('Invalid NFT data');
  await initializeSchema();
  const userAddress = nft.user_address.toLowerCase();
  await sql`
    INSERT INTO nfts (id, user_address, token_id, title, rarity, minted_at, image)
    VALUES (${nft.id}, ${userAddress}, ${nft.token_id}, ${nft.title || null}, ${nft.rarity || 'common'}, ${nft.minted_at}, ${nft.image || null})
